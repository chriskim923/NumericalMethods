// Runge-Kutta (4th Order) Method to Solving First-Order Diff EQ
//
...
double f2(double t, double x1, double x2, double x3, double x4);	// the right hand side of the ODE dx/dt = f2(t,x)
double f3(double t, double x1, double x2, double x3, double x4);	// the right hand side of the ODE dx/dt = f3(t,x)
double f4(double t, double x1, double x2, double x3, double x4);	// the right hand side of the ODE dx/dt = f4(t,x)
...
int main() {
    ...
    double k11, k21, k31, k41; // RK parameters for x1
    double k12, k22, k32, k42; // RK parameters for x2
    double k13, k23, k33, k43; // RK parameters for x3
    double k14, k24, k34, k44; // RK parameters for x4
    
    double x10 = 0.; // initial condition
    double x1 = x10;

    double x20 = 1.; // initial condition
    double x2 = x20;
    
    double x30 = 0.; // initial condition
    double x3 = x30;
    
    double x40 = 0.; // initial condition
    double x4 = x40;
    
    data_out<<"t"<<'\t'<<"x1"<<'\t'<<"x2"<<endl; // headers
